...
# v12: import game_array3 to fix bug around corner diagonal case!
       policy10a to allow loading weights in CPU
# v11id4: import slap6b to fix bug in cc_state & cc_pos in refactoring slap5
  (not affect n, s models, only affect groupiods/c models using slap6, if any)  
# v11id3: allow stage0_duration be used for cases other than use_slap == 'replace'
# v11id2_report2: same code as previous, diff config to tune n0 models
# v11id2_report: same code as v11id2, diff config for final report model
...
import itertools
from slap6b import slap, slap_opening, cc_state, cc_pos, stone_pos
import csv
import os
import sys
import subprocess
...
class TrainPipeline():
    def __init__(self, init_model=None, init_buffer=None):
        ...
        self.L2 = 1e-3               # coef of L2 penalty
        ...
        self.buffer_size_later = 40000   #buffer size at second stage, won't auto-scale down by 8
        self.batch_size = 512         # mini-batch size for training
        self.play_batch_size = 1      # no of games in a batch
        self.stage0_duration = 1000    # early stage duration by no of games; enter large num if skip second stage; enter 0 if skip stage0 i.e. want full batch size and full epoch for all stages
        ...
        self.game_batch_num = 5000       # end batches of games
        ...
        self.trial_run_name = 'v12_n0(3)_Adam_0.1noise0.4_0.3D_0.25expl_0.001lr_0.001L2_0drp_10000buffer40000_10e'
        ...
        self.noise = (0.1, 0.4)     #weight of randomness, e.g.(0.1, 0.4) means weight sliding over 0.1 to 0.4 against leaf value
        ...
        self.use_slap  = False        #'add', 'replace' or False, supplement or replacement of data augmentation
    ...
    def run(self):
        ...
        try:
            ...
            for i in range(self.start_i, int(self.game_batch_num)):
                if (i == self.start_i + self.stage0_duration):    #v11id3: remove "and self.use_slap == 'replace':"
